...
class TCP_Header:
    ...
    def flags_set(self,ack, rst, syn, fin, psh):
        self.flags["ACK"] = ack
        self.flags["RST"] = rst
        self.flags["SYN"] = syn
        self.flags["FIN"] = fin
        self.flags["PSH"] = psh  # Add PSH flag to the dictionary
    ...
    def get_flags(self,buffer):
        ...
        syn = (value & 2)>>1
        rst = (value & 4)>>2
        psh = (value & 8)>>3  # Extract PSH flag
        ack = (value & 16)>>4
        self.flags_set(ack, rst, syn, fin, psh)
        return None
